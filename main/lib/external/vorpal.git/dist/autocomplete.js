...
function getMatchData(input, cb) {
  ...
  var cmd = input.match;
  // console.log('getMatchData::input => ' + customStringify(input, null, 2));
  ...
  {
      var re = /--(\w+) (\S+)/i;
      var found = optionStr.match(re);
      return [found[1], found[2]];
  }

  function extractContext() {
      var re = /(?:--(\w+) (\S+))+/ig;
      var found = input.raw.match(re);
      var currentOptionName = input.option;
      currentOptionName = _.trimStart(currentOptionName, '-');
      var ctx = {
          currentName: currentOptionName,
          currentValue: '',
          options: {}
      };
      if (found) {
          for(var x of found) {
              var parsedOption = parseOption(x);
              if (parsedOption) {
                  ctx.options[parsedOption[0]] = parsedOption[1];
              }
          }
      }
      if (currentOptionName) {
          ctx.currentValue = ctx.options[currentOptionName];
          delete ctx.options[currentOptionName];
      }
      return ctx;
  }
  ...
  function handleDataFormat(str, config, callback) {
    ...
    if (_.isArray(config)) {
      ...
      var res = config(str, extractContext(), cbk);
