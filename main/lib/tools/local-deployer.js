...
class LocalDeployer
{
    constructor(logger, configRegistry, docker)
    {
        this._logger = logger;
        this._docker = docker;
    ...
    deployClusters(clusterNameOrNone, screen)
    {
        ...
        if (clusterNameOrNone)
        {
            ...
            {
                screen.write('Cluster ' + clusterNameOrNone + ' not present.');
                return;
            }
        }
        else
    ...
    _processDirtyCluster(clusterName)
    {
        ...
        if (isDeployed)
        {
            return this._processCluster(clusterName, 'single-stage-deploy');
        }
        else
        {
            return this._processCluster(clusterName, 'single-stage-undeploy');
        }
    }
    ...
    _processCluster(clusterName, stage)
    {
        ...
        return Promise.resolve()
            ...
            .then(clusterEntity => {
                var processor = new LocalProcessor(this._logger.sublogger('LocalProc'), this._repoStore, this._docker);
