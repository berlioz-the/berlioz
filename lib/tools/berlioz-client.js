...
class BerliozClient
{
    constructor(logger, screen, configRegistry, environment)
    {
        this._screen = screen;
        ...
        var endpointOverride = this._environment.getValue('BERLIOZ_ENDPOINT_OVERRIDE');
        if (endpointOverride) {
            var fileContentsStr = fs.readFileSync(endpointOverride);
    ...
    _rawRequest(method, baseUrl, path, data, token)
    {
        ...
        return request(options)
            ...
            .catch(err => {
                ...
                this._logger.info('[_rawRequest] ERROR:', err);
                this._massageErrorResponse(err);
                this._logger.error('[_rawRequest] RESPONSE ERROR: %s', err.message);
                this._logger.error('[_rawRequest] URL: %s', options.uri);
                this._logger.error('[_rawRequest] ALL OPTIONS: ', options);
