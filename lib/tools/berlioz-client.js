...
class BerliozClient
{
    ...
    _loginToRegion(username, password, region)
    {
        ...
        return Promise.resolve()
            ...
            .then(() => this._requestToRegion(region, 'POST', SECTION_AAA, '/login', { username: username, password: password }))
            .then(result => {
                if (result) {
                    if (result.success) {
                        return Promise.resolve(this._resolveRegionBaseUrl(region))
                            .then(baseUrl => {
                                this._configRegistry.set('auth', ['token', baseUrl], result.id_token);
                            })
                            .then(() => result);
                    }
                }
    ...
    _requestToRegion(region, method, baseSection, path, data)
    {
        return Promise.resolve(this._resolveRegionBaseUrl(region))
            .then(baseUrl => {
                ...
                var token = this._configRegistry.get('auth', ['token', baseUrl]);
                return this._rawRequest(method, targetUrl, path, data, token);
            })
    }
