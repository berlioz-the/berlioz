...
class Deployer
{
    ...
    _setupTargetVersions()
    {
        ...
        {
            this._setupTargetVersion(deploymentCluster);        
        }
    }

    _setupTargetVersion(deploymentCluster)
    {
        this._logger.info('[_setupTargetVersion]: ', deploymentCluster);

        if (!deploymentCluster.isImplicit) {
    ...
    _outputChangesToBeMade()
    {
        ...
        this._screen.header('CLUSTERS MATCHING SEARCH CRITERIA');
        this._screen.table()
            .autofitColumn('Deployment')
            .autofitColumn('Cluster')
