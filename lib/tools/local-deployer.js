...
class LocalDeployer
{
    constructor(logger, configRegistry, docker, screen, shell, environment, storage)
    {
        ...
        return Promise.resolve()
            .then(() => {
                if (dirtyInfo.postpone) {
                    ...
                    if (diffSec > 0) {
                        ...
                        if (dirtyInfo.needMoreReasons) {
                            for(var x of dirtyInfo.needMoreReasons)
                            {
                                this._screen.info(x);
                            }
                        }
            ...
            .then(registry => {
                ...
                var processor = new LocalProcessor(
                    this._logger.sublogger('LocalProc'), 
                    this._repoStore, 
                    this._docker, 
                    this._screen, 
                    this._shell, 
                ...
                processor.setEndpointProcessor(endpointProcessor);
                processor.setMetadataProcessor(metadataProcessor);
                processor.setPeersFetcher(this._createPeersFetcher());
            ...
            .then(result => {
                ...
                if (result.needMore) {
                    ...
                    if (result.needMoreReasons) {
                        var reasons = _.uniq(result.needMoreReasons);
                        this._markClusterNeedMoreReasons(clusterName, reasons);
                    }
                }
    ...
    _markClusterNeedMoreReasons(clusterName, reasons)
    {
        this._logger.info('[_markClusterNeedMoreReasons] %s:', clusterName, reasons);
        this._repoStore.set('local-dirty-clusters', [clusterName, 'needMoreReasons'], reasons);
    }
