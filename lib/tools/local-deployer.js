...
class LocalDeployer
{
    ...
    _processDirtyClusters()
    {
        ...
        return Promise.resolve()
            ...
            .then(() => {
                clusterNames = _.keys(this._repoStore.get('local-dirty-clusters', []));
            })
            .then(() => {
                // TODO;;; TEMPORARY
            ...
            .then(() => {
                // TODO replace this check wiht more optimal check.
            ...
            .then(() => {
                ...
                return Promise.serial(delta, x => {
                    return Promise.resolve()
                        .then(() => {
                            if (x.isPresent) {
                                this._repoStore.set('local-deployed-clusters', [x.name])
                            } else {
                                this._repoStore.delete('local-deployed-clusters', [x.name])
                            }
                        })
                        .then(() => this._markClusterDirty(x.name))
                })
            });
    }
    ...
    _hasExplicitClusters() 
    {
        ...
        for(var clusterInfo of _.values(clusters))
        {
