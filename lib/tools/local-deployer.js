...
class LocalDeployer
{
    constructor(logger, configRegistry, docker, screen)
    {
        this._logger = logger;
        this._docker = docker;
        this._screen = screen;
    ...
    _processCluster(clusterName, action)
    {
        ...
        return Promise.resolve()
            ...
            .then(clusterEntity => {
                var processor = new LocalProcessor(this._logger.sublogger('LocalProc'), this._repoStore, this._docker, this._screen);
