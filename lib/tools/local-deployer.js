...
class LocalDeployer
{
    constructor(logger, configRegistry, docker, screen, shell)
    {
        ...
        this._repoStore.setupRepository('local-endpoints-public', 'LOCAL PUBLIC ENDPOINTS');
        this._repoStore.setupRepository('local-endpoints-external', 'LOCAL EXTERNAL ENDPOINTS');
        this._repoStore.setupRepository('local-endpoints-internal', 'LOCAL INTERNAL ENDPOINTS');
        this._repoStore.setupRepository('local-priority-endpoints-public', 'LOCAL PRIORITY PUBLIC ENDPOINTS');
        this._repoStore.setupRepository('local-priority-endpoints-external', 'LOCAL PRIORITY EXTERNAL ENDPOINTS');
        this._repoStore.setupRepository('local-priority-endpoints-internal', 'LOCAL PRIORITY INTERNAL ENDPOINTS');
        this._repoStore.setupRepository('local-cluster-dependencies', 'LOCAL CLUSTER DEPENDENCIES');
        ...
        this._clusterDependentFetcher = new TableFetcher(
            logger.sublogger("TableFetcher"), 
    ...
    outputClusterEndpoints(clusterName)
    {
        return this._loadClusterEntity(clusterName)
            .then(clusterEntity => {
                return Promise.serial(_.values(clusterEntity.provides), x => this._outputClusterProvided(x))
            });
    }

    _outputClusterProvided(clusterProvided)
    {
        return Promise.resolve(this._peersFetcher._prefetchClusterProvides(clusterProvided))
            .then(() => {
                this._screen.header('Cluster %s, Endpoint: %s', clusterProvided.cluster.name, clusterProvided.name);

                var endpointMap = this._peersFetcher.getClusterPublicEndpoints(clusterProvided);
                // cluster, endpoint, entryMap
    ...
    _processCluster(clusterName, action)
    {
        ...
        return Promise.resolve()
            ...
            .then(() => {
                ...
                return Promise.serial(_.keys(this._dirtyEndpointServices), x => this._processDirtyEndpointService(x));
            })
            ;
    }
    ...
    _processDirtyEndpointService(serviceId)
    {
        this._logger.info("[_processDirtyEndpointService] %s ", serviceId);
        return this._clusterDependentFetcher.query({ dependent: serviceId })
            .then(results => {
