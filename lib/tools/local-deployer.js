...
const Compiler = require('../berlioz-common/compiler');
                ...
                if (stage == 'single-stage-deploy') {
                    ...
                    return Promise.resolve(this._parseDefinitions(clusterDefinitions))
                        .then(registry => {
                            ...
                            var compiler = new Compiler(this._logger);
                            return compiler.process(registry);
                        })
                        .then(registry => {
                            var clusterEntity = null;
                            if (registry) {
                            ...
                            if (!clusterEntity) {
                                this._logger.info('Failed to compile registry for cluster: %s.', clusterName);
                                return;
                            }
                            policyEntities = registry.policies;
                            return clusterEntity;
                        });
                } else {
                    return Promise.resolve(this._parseDefinitions([{ kind: 'cluster', name: clusterName}]))
                        .then(registry => {
                            return registry.getCluster(clusterName);
                        })
                }
            })
            .then(clusterEntity => {
                ...
                processor.policies(policyEntities);
