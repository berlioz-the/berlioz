...
class Screen
{
    ...
    outputService(service)
    {
        ...
        if (_.values(service.secretsConsumes).length > 0) {
            this.header('SERVICE SECRETS CONSUMED %s :: %s', service.clusterName, service.name);
            this.invertedTable(_.values(service.secretsConsumes).map(x => {
                var data = [];
                x.extractData(data);
                return data;
            }), ['id', 'dn', 'kind', 'path']).output();
