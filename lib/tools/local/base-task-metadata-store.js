...
class BaseTaskMetadataStore
{
    ...
    get peersFetcher() {
        return this._rootProcessor._peersFetcher;
    }
    ...
    processTask(task)
    {
        ...
        return Promise.resolve()
            ...
            .then(() => {
                ...
                var consumedMap = this.rootProcessor.nativeProcessor.extractServiceConsumedItems(service);
                for(var consumedItem of _.values(consumedMap)) {
                    if (consumedItem.item) {
                        var peer = _.clone(consumedItem.info);
                        peer.name = consumedItem.item.id;
                        if (consumedItem.consumedItem) {
                            peer.subName = consumedItem.consumedItem.id;
                        }
                        peers[consumedItem.info.id] = {
                            "0" : peer
                        };
                    }
                }
