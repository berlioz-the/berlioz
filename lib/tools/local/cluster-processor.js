...
class ClusterProcessor
{
    ...
    getEncryptionKeyAlias(config)
    {
        ...
        var aliasNaming = [this.deploymentName, this.name];
    ...
    _constructRsaSecret(config, secret)
    {
        ...
        var namingPublic = [this.deploymentName, this.name, secret.name, 'secret_public_key'];
        var namingPrivate = [this.deploymentName, this.name, secret.name, 'secret_private_key'];
    ...
    _constructBerliozAgent(config)
    {
        ...
        var repoInfo = this.rootProcessor.getHelperImage('agent');
