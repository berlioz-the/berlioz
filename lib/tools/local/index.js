...
class LocalProcessor extends ModelProcessor
{
    constructor(logger, repoStore, docker, screen, shell)
    {
        ...
        this.setupStage('single-stage-deploy', () => {
            ...
            this._iterationStages.interationInit = ['prepare-common-images', 'iteration-init', 'setup-docker-network'] ;
        ...
        this.setupStage('single-stage-undeploy', () => {
            ...
            this._iterationStages.constructDesired = 'cleanup-metadata-processor';
            this._iterationStages.stabilizeCurrent = ['reserve-network-resources', 'massage-key-aliases'];
            // this._iterationStages.postExtractCurrent = null;
            this._iterationStages.postProcessDelta = ['massage-key-aliases', 'fetch-berlioz-agent-task', 'deploy-task-metadata'];
            return this.runStage('process-iteration');
    ...
    _fetchBerliozAgentTask()
    {
        ...
        return Promise.resolve()
            ...
            .then(containers => {
                ...
                this._berliozAgentExternalIpAddress = '127.0.0.1';
    ...
    _preSetup()
    {
        ...
        this._nativeProcessor = this._berliozCommon.newNativeProcessor(this._logger);
    ...
    _setupGcp()
    {
        ...
        var providerConfig = this._providers['gcp'];

        this._gcpClient = new GCPClient(
            this._logger.sublogger('AWSClient'),
            providerConfig.region,
            providerConfig.credentials);
        this._metaContext.gcp = this.gcpClient;
        this._metaContext.gcpAccountId = providerConfig.credentials.project_id;

        this._nativeProcessor.setupProviderPeerConfigOverride('gcp', {
            credentials: {
                client_email: providerConfig.credentials.client_email,
                private_key: providerConfig.credentials.private_key
            }
        });
        this._nativeProcessor.setupScope({
            providerKind: 'gcp',
            deployment: this.deploymentName,
            gcpAccountId: providerConfig.credentials.project_id,
            region: this.gcpClient.rawRegion
        })
    }
    ...
    _setupAws()
    {
        ...
        var providerConfig = this._providers['aws'];

        if (providerConfig.profile) {
            this._screen.info('Using AWS profile: %s', providerConfig.profile);
        } else {
        ...
        this._awsClient = new AWSClient(
            providerConfig.region,
            providerConfig.credentials, 
            this._logger.sublogger('AWSClient'));
        this._metaContext.aws = this.awsClient;

        this._nativeProcessor.setupProviderPeerConfigOverride('aws', {
        ...
        this._nativeProcessor.setupScope({
            providerKind: 'gcp',
            deployment: this.deploymentName,
            // awsAccountId: providerConfig.credentials.project_id,
            region: providerConfig.region
        })
    }
