...
{
    constructor(logger, repoStore, docker, screen, shell, environment, storage)
    {
        super(logger);
        ...
        this.setupStage('single-stage-deploy', () => {
            ...
            this._iterationStages.preProcessDelta = ['output-other-config'];
        ...
        this.setupStage('output-other-config', () => {
            return this._outputOtherConfig();
        });
    }
    ...
    setEndpointProcessor(value)
    {
        this._endpointProcessor = value;
    }

    setPeersFetcher(value)
    {
    ...
    get endpointProcessor() {
        return this._endpointProcessor;
    }
    ...
    getRootWorkingPath()
    {
        return this._storage.getConfigDir("deployment");
    }
    ...
    getNewDeploymentWorkingPath()
    {
        var p = this._storage.getConfigDir("deployment-config"); //this.getRootWorkingPath();
        var p = Path.resolve(p, this.clusterName); 
        return p;
    }

    getTaskMetadataWorkingPath(subdirOrNone)
    {
        var p = this._storage.getConfigDir("deployment-config"); //this.getRootWorkingPath();
        p = Path.resolve(p, this.clusterName); 
        if (subdirOrNone) {
            p = Path.resolve(p, subdirOrNone); 
        }
        return p;
    }


    // getItemWorkingPath(item, otherPathParts)
    // {
    //     var p = this.getRootWorkingPath();
    //     var p = Path.resolve(p, this.clusterName); 
    ...
    //     myName = myName.replace(/\//g, "");
    //     myName = myName.replace(/:/g, "-");
    //     var p = Path.resolve(p, myName); 

    //     if (otherPathParts) {
    //         for(var x of otherPathParts)
    //         {
    //             p = Path.resolve(p, x); 
    //         }
    //     }
    ...
    getServiceEndpointWorkingPath(serviceEntity, endpoint)
    {
        return Path.resolve(this.getServiceWorkingPath(serviceEntity), "endpoints", endpoint);
    }

    writeToFile(filePath, contents)
    {
        return this.storage.writeToFile(filePath, contents);
    }

    readFromFile(filePath)
    {
        return this.storage.readFromFile(filePath);
    }
    ...
    addToDeploymentConfig(name, path, contents)
    {
        var deploymentConfig = this._getDeploymentConfig(name);
        deploymentConfig.config.contents[path] = contents;
    }

    _getDeploymentConfig(name)
    {
        var naming = [name];
        var x = this._desiredConfig.find('deployment-config', naming);
        if (x) {
            return x;
        }
        x = this._desiredConfig.section('deployment-config').create(naming);
        x.config.contents = {
    ...
    {
        return Promise.resolve()
            .then(() => this.nativeProcessor.debugWriteConfig())
            ;
    }
    ...
    getContainerIp(task)
    {
        ...
        return _.get(task.obj, "NetworkSettings.Networks.berlioz.IPAddress", null);
    }
    ...
    _setupGcp()
    {
        ...
        this._metaContext.sourceRegion = this.gcpClient.sourceRegion;
        ...
        this._nativeProcessor.setupProviderPeerConfigHandler('gcp', (item) => {
            var resolvedItem = item.resolved;
            if (resolvedItem) {
                if (resolvedItem.meta.name == 'gcp-sql') {
                    if (resolvedItem.runtime) {
                        if (resolvedItem.runtime.host) {
                            return {
                                host: resolvedItem.runtime.host
                            }
                        }
                    }
                    return {};
        ...
        var nativeProcessorScope = {
            ...
            sourceRegion: this._metaContext.sourceRegion,
            shortSourceRegion: this._metaContext.shortSourceRegion,
            region: this._metaContext.region,
