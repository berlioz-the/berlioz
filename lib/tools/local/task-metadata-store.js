...
class TaskMetadataStore extends BaseTaskMetadataStore
{
    ...
    extract()
    {
        return Promise.resolve()
            ...
            .then(() => this._peersFetcher.prefetchClusterDependencies(this._clusterEntity))
            .then(() => {
    ...
    _extractTaskPolicies(taskInfo, task, service)
    {
        ...
        var dtraceConfig = service.resolvePolicy("distributed-tracing-provider");
        if (dtraceConfig.value == 'zipkin' || dtraceConfig.value == 'jaeger') {
            policies['enable-zipkin'] = true;

            var zipkinSvc = BaseItem.constructID('service', [this._clusterEntity.name, 'infra', 'dtrace']);
            policies['zipkin-service-id'] = [zipkinSvc, 'client'].join('-');
        }
    ...
    _extractLoadBalancerEndpoints(loadBalancer)
    {
        ...
        endpointsMeta.internal.address = this._rootProcessor.getContainerIp(loadBalancer.obj);
