...
class TaskMetadataStore extends BaseTaskMetadataStore
{
    ...
    _extractTaskPolicies(taskInfo, task, service)
    {
        ...
        {
            var zipkinTask = this._currentConfig.find('task', [task.naming[0], 'zipkin', 0]);
            if (zipkinTask) {
                var zipkinIp = this._rootProcessor.getContainerIp(zipkinTask.obj);
                if (zipkinIp) {
                    policies['zipkin-endpoint'] = 'http://' + zipkinIp + ':9411/api/v2/spans';;
                }
            }
        }
