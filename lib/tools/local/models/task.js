...
module.exports = (section, logger, {docker, helper, cluster, screen}) => {
    ...
    section
        ...
        .onAutoConfig((item, action) => {
            ...
            var zipkinTask = _(item.findRelations('task')).map(x => x.targetItem).find(x => x.config.isZipkin);
            if (zipkinTask) {
                var zipkinIp = helper.getContainerIp(zipkinTask.obj);
                if (zipkinIp) {
                    item.config.environment['BERLIOZ_ZIPKIN_PATH'] = 'http://' + zipkinIp + ':9411/api/v2/spans';
                }
            }

            return item.config.environment['BERLIOZ_AGENT_PATH'] && item.config.environment['BERLIOZ_ZIPKIN_PATH'];
        })
        .onCreate(delta => {
