...
module.exports = (section, logger, {docker, helper, cluster, screen}) => {
    ...
    section
        ...
        .onExtractRelations(item => {
            ...
            var taskMetadataName = 'task-' + item.naming.join('-');
            item.relation('task-metadata', [taskMetadataName])
                .setupTargetAutoCreate();
        })
    ...
    function startTask(delta)
    {
        ...
        }
        {
            ...
            parts = parts.map(x => sanitizeHostNamePart(x));
    ...
    function sanitizeHostNamePart(str)
    {
