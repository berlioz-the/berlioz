...
{
    constructor(logger, executeCb, autocompleteProcessor)
    {
        this._logger = logger;
    ...
    accept(info)
    {
        ...
        var commandModule = require(info.includePath);
        ...
        var descrFilePath = Path.join(info.dirPath, 'description');
        if (fs.existsSync(descrFilePath))
        {
            var descrContents = fs.readFileSync(descrFilePath, {encoding: 'utf8'});
