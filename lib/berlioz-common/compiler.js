...
class Compiler
{
    ...
    _processStage(stage)
    {
        ...
        if (stage.clusterAction)
        {
            ...
            {
                if (stage.canRun) {
                    if (!stage.canRun({compiler: this, registry: this._registry, cluster: cluster, logger: this._logger})) {
                        continue;
                    }
                }
                this._logger.info('[Compiler::process] %s, running %s cluster action...', stage.name, cluster.name);
                stage.clusterAction({compiler: this, registry: this._registry, cluster: cluster, logger: this._logger})
            }
        }
    ...
    _processStageGlobal(stage)
    {
        ...
        if (stage.canRun) {
            if (!stage.canRun({compiler: this, registry: this._registry, logger: this._logger})) {
                return;
            }
        } 

        this._logger.info('[Compiler::process] %s, running global action...', stage.name)
        stage.globalAction({compiler: this, registry: this._registry, logger: this._logger})
    }
