...
class Compiler
{
    constructor(logger)
    {
        ...
        this._stages = [
            ...
            {
                name: "FinalizePrometheus",
                action: this._finalizePrometheus.bind(this)
            }
        ]
    }
        ...
        for(var cluster of registry.clusters)
        {
            ...
            if (registry.findById(prometheusId))
            {
                for(var sector of cluster.sectors)
                {
                    ...
                    if (registry.findById(agentServiceId))
                    {
                        this._addConsumes(prometheusId, {
                            sector: sector.name,
                            service: 'berlioz_agent',
    ...
    _extendGrafana(registry)
    {
        for(var cluster of registry.clusters)
        {
            var grafanaId = this._addImpicit({
                kind: 'service',
                sector: 'infra',
                cluster: cluster.name,
                name: 'grafana',
                code: {
                    kind: 'docker',
                    image: 'berliozcloud/grafana' //berliozcloud
                },
                provides: {
                    default: {
                        port: 3000,
            ...
            this._addClusterProvided(cluster.id, "grafana", {
                service: 'grafana',
                sector: 'infra',
                public: true
            })
