...
class NativeProcessor
{
    ...
    constructConfig(config, clusterEntity)
    {
        ...
        return Promise.resolve()
            .then(() => this._setupProviderHelper(config))
            .then(() => {
                if (!this._providerHelper) {
                    return;
                }
                if (!this._providerHelper.constructConfig) {
                    return;
                }
                return this._providerHelper.constructConfig();
            })
    ...
    _setupProviderHelper(config)
    {
        ...
        var handlerArgs = this._makeHandlerArgs(config, null);
    ...
    _makeHandlerArgs(config, handlerArgs)
    {
        ...
        if (this._providerHelper) {
            handlerArgs.providerHelper = this._providerHelper;
        } else {
