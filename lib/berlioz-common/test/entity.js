...
describe('entity.js', function() {
    ...
    describe('resolvePolicy', function() {
        ...
        it('sample3-scale-policy-local-frontend', function () {
            return require('./support/load-registry')('sample3')
                ...
                .then(registry => {
                    var service = registry.findById('service://fundme-main-frontend');
                    should.exist(service)

                    var policy = service.resolvePolicy('scale');
                    should.exist(policy)
                    should.exist(policy.config)
                    should.exist(policy.config.desired)
                    policy.config.desired.should.be.exactly(4)
                });
        });

        it('sample3-scale-policy-prod-frontend', function () {
            return require('./support/load-registry')('sample3')
                .then(registry => {
                ...
                .then(registry => {
                    ...
                    should.exist(policy.config)
                    should.exist(policy.config.desired)
                    policy.config.desired.should.be.exactly(3)
                });
        });
        
        it('sample3-scale-policy-test-frontend', function () {
            return require('./support/load-registry')('sample3')
                .then(registry => {
                ...
                .then(registry => {
                    ...
                    should.exist(policy.config)
                    should.exist(policy.config.desired)
                    policy.config.desired.should.be.exactly(2)
                });
        });
