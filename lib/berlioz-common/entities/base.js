...
class Base
{
    ...
    extractData(data)
    {
        ...
        data.push(['path', this.berliozfile]);
    ...
    link(obj, nameOverride)
    {
        ...
        var myLinks = this.getLinks(name);
        // for (var x of myLinks) {
        //     if (x.id == obj.id) {
        //         throw new Error(`Link to ${obj.id} already exists. Myself=${this.id}`);
        //     }
        // }
        myLinks.push(obj);
    }
    ...
    _myEnvironment() {
        ...
        return this._mergeEnvironment([
            ...
            {
                'BERLIOZ_CONSUMES_PATH': this.berliozConsumesPath
            }
        ]);
    }
    ...
    findConsumer(consumerInfo)
    {
        if (!this.cluster) {
            throw new Error("Does not have cluster reference");
        }

        var sectorName = this.sectorName;
        if (consumerInfo.sector) {
            sectorName = consumerInfo.sector;
        }

        var entities = null;
        if (consumerInfo.database) {
            entities = this.cluster.databases.filter(x => x.sectorName == sectorName && x.name == consumerInfo.database);
        }
        if (consumerInfo.lambda) {
            entities = this.cluster.lambdas.filter(x => x.sectorName == sectorName && x.name == consumerInfo.lambda);
        }
    ...
    validateSyntax(validator)
    {
        var schema = this._getValidationSchema(Joi);
        if (schema) {
            var result = Joi.validate(this.definition, schema);
            if (result.error)
            {
                validator.submitItemJoiError(this, result.error);
            }
        }

        this._customValidateSyntax(validator);
    }
    ...
    _customValidateSyntax(validator)
    {
    ...
    getNativeConfig()
    {
        var config = this.resolvePolicy('native-config', [this.kind]);
        if (config) {
