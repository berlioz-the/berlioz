...
class Service extends DockerBased
{
    ...
    get berliozConsumesPath()
    {
        return '/etc/berlioz/consumes';
    }
    ...
    _getValidationSchema(Joi)
    {
        return Joi.object().keys({
            ...
            storage: Joi.array().items(Joi.object().keys({
                    kind: Joi.string().valid('volume', 'bind'),
            ...
            consumes: Joi.array().items(Joi.object().keys({
                    ...
                    sector: Joi.sectorName(),
                    actions: Joi.array(),
                    isolation: Joi.string().valid('none', 'instance'),
                    meta: Joi.boolean()
                })
            ...
            sidecar: Joi.string().valid('none', 'instance')
        });
    }
    // #endregion
