...
class Starter
{
    ...
    _executeCommand(info, commandModule, args)
    {
        ...
        try {
            ...
            if (this.needsDocker(commandModule)) {
                commandModule.docker = new Docker(this._logger.sublogger('Docker'), this._screen, commandModule.shell, this._environment);
                commandParams.docker = commandModule.docker;
            }

            if (commandModule.useLocalDeployer)
            {
                commandParams.localDeployer = new LocalDeployer(this._logger.sublogger('LocalDeployer'), this._configRegistry, commandModule.docker, this._screen, commandModule.shell, this._environment);
            }
    ...
    {
        this._logger.info("[_loadDefinitions] pathOverride=", pathOverride)
        var registry = null;
        var dirs = null;
        if (pathOverride) {
